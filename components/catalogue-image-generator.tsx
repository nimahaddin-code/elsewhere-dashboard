...
export default function CatalogueImageGenerator({
  ...
  const generate = async () => {
    ...
    try {
      ...
      first.ctx.fillText(theme.label, SIZE.width / 2, 1144);
      first.ctx.letterSpacing = "0px";
      first.ctx.strokeStyle = theme.accent;
      first.ctx.lineWidth = 2;
      roundedRect(first.ctx, 330, 1166, 420, 92, 46);
      first.ctx.stroke();
      first.ctx.font = "500 25px Arial, sans-serif";
      first.ctx.letterSpacing = "6px";
      const singleProduct = loaded.length === 1;
      first.ctx.fillText(singleProduct ? `PRICE  RP${sellingPrice(loaded[0].variant).toLocaleString("id-ID")}` : `START FROM  ${shortPrice(roundedStart(minPrice))}`, SIZE.width / 2, 1223);
